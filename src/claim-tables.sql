...
DECLARE @GlobalAppScopeId UNIQUEIDENTIFIER
DECLARE @AppScope UNIQUEIDENTIFIER
DECLARE @DashboardScope UNIQUEIDENTIFIER

SELECT @GlobalAppScopeId = ClaimScopeId from ClaimScopes where [Key] = 'application-global'
SELECT @AppScope = ClaimScopeId from ClaimScopes where [Name] = 'SPA Application'
SELECT @DashboardScope = ClaimScopeId from ClaimScopes where [Key] = 'dashboard'

INSERT INTO [dbo].[RoleTypes]([ClaimScopeId], [Value])
VALUES
...
SELECT @UserId = Id from AspNetUsers where Email = 'susan.smith@gmail.com'

INSERT INTO [dbo].[ClaimUserValues](UserId, ClaimScopeId, ClaimTypeId, [Value])
VALUES
    (@UserId, @GlobalAppScopeId, 1, 'USA'),
    (@UserId, @AppScope, 8, '15068'),
    (@UserId, @AppScope, 2, '1972-12-16'),
    (@UserId, @DashboardScope, 11, 'Admin')

select * from ClaimScopes
select * from ClaimTypes
select * from RoleTypes
select * from ClaimUserValues
